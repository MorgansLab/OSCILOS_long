function varargout = GUI_FREQ_EigCal_nonlinear_HR(varargin)
% GUI_FREQ_EIGCAL_NONLINEAR_HR MATLAB code for GUI_FREQ_EigCal_nonlinear_HR.fig
%      GUI_FREQ_EIGCAL_NONLINEAR_HR, by itself, creates a new GUI_FREQ_EIGCAL_NONLINEAR_HR or raises the existing
%      singleton*.
%
%      H = GUI_FREQ_EIGCAL_NONLINEAR_HR returns the handle to a new GUI_FREQ_EIGCAL_NONLINEAR_HR or the handle to
%      the existing singleton*.
%
%      GUI_FREQ_EIGCAL_NONLINEAR_HR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_FREQ_EIGCAL_NONLINEAR_HR.M with the given input arguments.
%
%      GUI_FREQ_EIGCAL_NONLINEAR_HR('Property','Value',...) creates a new GUI_FREQ_EIGCAL_NONLINEAR_HR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_FREQ_EigCal_nonlinear_HR_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_FREQ_EigCal_nonlinear_HR_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_FREQ_EigCal_nonlinear_HR

% Last Modified by GUIDE v2.5 06-Nov-2014 14:49:17

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_FREQ_EigCal_nonlinear_HR_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_FREQ_EigCal_nonlinear_HR_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_FREQ_EigCal_nonlinear_HR is made visible.
function GUI_FREQ_EigCal_nonlinear_HR_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_FREQ_EigCal_nonlinear_HR (see VARARGIN)
% --------------------------------------------------------------------------
global CI
handles.indexEdit = 1;
    ...
    case 0
        ...
        if dontOpen
           ...
           disp('-----------------------------------------------------');
            ...
            if CI.indexFM == 0      % From experimental FDF
                uiwait(hObject)
            else
                errordlg('Flame describing function must be an analytical expression','Error'); 
            end
        end
    case 1
...
%         CI.IsRun.GUI_FREQ_EigCal_nonlinear_HR = 0;
        ...
        guidata(hObject, handles);  
        GUI_FREQ_EigCal_Initialization(hObject, eventdata, handles)
%         indexExist_indexFM = any(strcmp('indexFM',fieldnames(CI)));
        if CI.indexFM == 0      % From experimental FDF
            uiwait(hObject)
        else
            errordlg('Flame describing function must be an analytical expression','Error'); 
...
set(handles.uipanel_INI,...
                        ...
                        'Title','Set inlet amplitude factor range x 100',...
                        'visible','on',...
                        'highlightcolor',handles.bgcolor{3},...
                        'borderwidth',1,...
                        'fontsize',handles.FontSize(2),...
...
set(handles.text_min,...
                        ...
                        'fontsize',handles.FontSize(2),...
                        'string','Minimum value (p1'')/p1_bar: [-]',...
                        'backgroundcolor',handles.bgcolor{3},...
                        'horizontalalignment','left');                         
...
set(handles.text_max,...
                        ...
                        'fontsize',handles.FontSize(2),...
                        'string','Maximum value (p1'')/p1_bar : [-]',...
                        'backgroundcolor',handles.bgcolor{3},...
                        'horizontalalignment','left');                         
...
set(handles.text_SampNum,...
                        ...
                        'fontsize',handles.FontSize(2),...
                        'string','Number of amplitude ratio samples: [-]',...
                        'backgroundcolor',handles.bgcolor{3},...
                        'horizontalalignment','left');                         
...
set(handles.text_Table,...
                        ...
                        'fontsize',handles.FontSize(2),...
                        'string','Eigenvalues for selected inlet amplitude factor',...
                        'backgroundcolor',handles.bgcolor{3},...
                        'horizontalalignment','left'); 
...
set(handles.text_uRatio,...
                        ...
                        'fontsize',handles.FontSize(2),...
                        'string','Selected inlet amplitude factor: [-]',...
                        'backgroundcolor',handles.bgcolor{3},...
                        'horizontalalignment','left');                         
...
handles = guidata(hObject);
global CI
switch CI.IsRun.GUI_FREQ_EigCal_nonlinear_HR
    case 0
    ...
    case 1 
        ...
        set(handles.slider_uRatio,...
                        'Enable','on',...
                        'min',1,...
                        'max',CI.EIG.PR.pRatioNum,...
                        'value',1,...
                        'SliderStep',[1/(CI.EIG.PR.pRatioNum-1), 1/(CI.EIG.PR.pRatioNum-1)]);
        set(handles.edit_uRatio,'string',num2str(CI.EIG.PR.pRatioSp(1)));
        set(handles.pop_numMode,    'enable','on');  
        set(handles.pop_PlotType,   'enable','on'); 
        set(handles.pb_AOC_Plot,         'enable','on');
        set(handles.edit_min,       'string',num2str(CI.EIG.PR.pRatioSp(1)));
        set(handles.edit_max,       'string',num2str(CI.EIG.PR.pRatioSp(end)));
        set(handles.edit_SampNum,   'string',num2str(CI.EIG.PR.pRatioNum));
...
function varargout = GUI_FREQ_EigCal_nonlinear_HR_OutputFcn(hObject, eventdata, handles) 
...
if isnan(uRatio_min)
    set(handles.edit_min, 'String', 1e-3);  % The minimum value is 1 Pa
    errordlg('Input must be a number','Error');
end
if uRatio_min<=1e-5
    set(handles.edit_min, 'String', 1e-3);
end
...
set(handles.slider_uRatio,...
                        ...
                        'value',1,...
                        'SliderStep',[1/(CI.EIG.PR.pRatioNum-1), 1/(CI.EIG.PR.pRatioNum-1)]);
set(handles.edit_uRatio,'string',num2str(CI.EIG.PR.pRatioNum(1)));

indexShow=1;
CI.EIG.PR.indexShow=indexShow;
assignin('base','CI',CI);
% ---------------update the slider---------------
set(handles.pop_numMode,        'enable','on');  
set(handles.pop_PlotType,       'enable','on');
...
%
% -------------------- pRatio function information --------------
%
%  
pRatioMin           = str2double(get(handles.edit_min,'string'))./100;
pRatioMax           = str2double(get(handles.edit_max,'string'))./100;
pRatioNum           = str2double(get(handles.edit_SampNum,'string'));
CI.EIG.PR.pRatioSp  = linspace(pRatioMin,pRatioMax,pRatioNum);
CI.EIG.PR.pRatioNum = pRatioNum;
CI.EIG.PR.R1abs     = abs(polyval(CI.BC.num1,0)./polyval(CI.BC.den1,0));
CI.EIG.PR.A1minusSp = CI.EIG.PR.pRatioSp.*CI.TP.p_mean(1,1)./(1+CI.EIG.PR.R1abs);
%
assignin('base','CI',CI);
%
% -------------------------------------------------------------------------
...
set(handles.edit_uRatio,'string',num2str(CI.EIG.PR.pRatioSp(indexShow)));
%
...
% handles    structure with handles and user data (see GUIDATA)
global CI
CI.IsRun.GUI_FREQ_EigCal_nonlinear_HR = 1;
assignin('base','CI',CI);
...
%  'Evolution of eigenvalue with velocity ratio'}
    ...
    case 3 %'Evolution of eigenvalue with velocity ratio'
        ...
        if pRatioNum==1
            ...
            for M_Num=1:1:Modes_Num 
                ...
                pRatio_num(M_Num)                       =Ratio_num_step;
                uRatio_mode(M_Num,1:1:pRatio_num(M_Num))=CI.EIG.PR.pRatioSp(1:1:pRatio_num(M_Num));
    ...
    x       = uRatio_mode(CI.EIG.pop_numMode,1:pRatio_num(CI.EIG.pop_numMode));
    EigFreq = EigFreq_mode(CI.EIG.pop_numMode,1:pRatio_num(CI.EIG.pop_numMode));
    EigGR   = EigGR_mode(CI.EIG.pop_numMode,1:pRatio_num(CI.EIG.pop_numMode));
...
for ss = 1:length(GRSp)
    ...
    for kk = 1:length(FreqSp)
        ...
        [x,fval,exitflag] = fsolve(@Fcn_DetEqn_nonlinear_HR,s0,options);                         % solve equation        
        if exitflag==1;
...
for ss = 1:n
    ...
    for kk  = 1:m
        ...
        s           = GR+1i*omega;
        F(ss,kk)    = Fcn_DetEqn_nonlinear_HR(s);
    end
end
%
% -------------------------end---------------------------------------------    
