function FIT = Fcn_Dhs_LowOrderFitting_Prod
close all
...
for kk = 1 : 7
    for ss = 1: length(TSp2)
        Dhs2(kk,ss) =  Fcn_Cal_Deltahs(TSp2(ss),1,kk,TF);
    end
    p2(kk,:) = polyfit(TSp2, Dhs2(kk,:),N);
    DhsFit2(kk,:) = polyval(p2(kk,:), TSp2);
    error2(kk) = mean( abs(Dhs2(kk,:) - DhsFit2(kk,:))./Dhs2(kk,:));
end
FIT.gas = {'CO2', 'CO', 'H2O', 'H2', 'O2', 'N2', 'Air'};
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
load('THERMO.mat')
THERMO.Prod.LowOrder.pDhs       = [p1,p2];
THERMO.Prod.LowOrder.errorDhs   = [error1',error2'];
THERMO.Prod.LowOrder.OrderDhs   = N;
